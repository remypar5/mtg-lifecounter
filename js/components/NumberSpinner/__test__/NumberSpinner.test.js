...
describe('<NumberSpinner />', () => {
    ...
    beforeEach(() => {
        component = shallow(<NumberSpinner value={20} />);
    });
    ...
    describe('changing the value', () => {
        ...
        it('should call the onChange event listener', () => {
            ...
            const componentWithOnChange = shallow(<NumberSpinner
                value={20}
                onChange={onChangeSpy}
            />);

            componentWithOnChange.find('SpinnerButton').at(1).simulate('press');
            expect(onChangeSpy).toHaveBeenCalledWith(21);
        });

        describe('limitations', () => {
            beforeEach(() => {
                component = shallow(<NumberSpinner
                    value={20}
                    min={19}
                    max={21}
                />);
